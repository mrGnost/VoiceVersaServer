...
def get_audio_process_src_path(filename, instance):
    filename = f'{uuid4()}.' + instance.split('.')[-1]
    return os.path.join("uploads/processing/src/", filename)
...
class Processing(models.Model):
    url = models.FileField(upload_to=get_audio_process_src_path)
    voice = models.IntegerField(default=230)
